...
def run(cfg):
        ...
        if sel_cases:
            for stem in sel_cases:
                ...
                if p.exists():
                    ...
                    print(f"[⚠️ Warning] File not found in cases: {p}")
        ...
        if sel_controls:
            for stem in sel_controls:
                ...
                if p.exists():
                    ...
                    print(f"[⚠️ Warning] File not found in controls: {p}")
    ...
    if not groups["cases"] and not groups["controls"]:
        raise RuntimeError("No dataset found. Review run.select or paths.datasets_root.")
